...
func fetchProfileData(db *sql.DB, email string)(profileStruct,error){
	sql_query := `SELECT id,email,username,dob,bio,created_at FROM profiles WHERE email = $1`
	row := db.QueryRow(sql_query,email)
	var profile profileStruct
	err := row.Scan(&profile.ID, &profile.Email,&profile.Username, &profile.DOB, &profile.Bio, &profile.CreatedAt)
	return profile,err
}
